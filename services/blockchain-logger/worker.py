...
EVENTS_FAILED = Counter("logger_events_failed_total", "Events that failed", ["reason"])
LOG_LATENCY = Histogram("logger_event_log_latency_seconds", "End-to-end latency",
                        buckets=[0.05, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 10.0])
CONSUMER_LAG = Gauge("logger_consumer_lag", "Approximate consumer lag")
...
class BlockchainLoggerWorker:
    ...
    def _process_message(self, raw_value: bytes) -> Optional[LogResult]:
        ...
        try:
            ...
            logger.error("Failed to deserialize: %s", exc)
            return None
        ...
        try:
            ...
            logger.error("IPFS upload failed for %s: %s", event.event_id, exc)
            raise
        ...
        try:
            ...
            logger.error("Fabric submission failed for %s: %s", event.event_id, exc)
            raise
        ...
        result = LogResult(
            event_id=event.event_id, ipfs_cid=cid, sha256=sha256,
            tx_id=tx_id, logged_at=datetime.utcnow(), duration_ms=duration_ms,
        )
        logger.info("Logged event %s: tx=%s cid=%s %.1fms", event.event_id, tx_id, cid, duration_ms)
        return result

    def _run_worker(self, worker_id: int):
        consumer = self._make_consumer()
        consumer.subscribe([config.kafka_topic_events])
        logger.info("Worker %d started on %s", worker_id, config.kafka_topic_events)
        try:
            while self._running:
                ...
                try:
                    ...
                    logger.error("Worker %d unrecoverable error: %s", worker_id, exc)
        finally:
            consumer.close()
    ...
    def stop(self):
        ...
        for t in self._threads:
            t.join(timeout=15)
        self._threads.clear()
        logger.info("All logger workers stopped")
