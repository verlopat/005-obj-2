...
LOG_LATENCY    = Histogram("logger_event_log_latency_seconds","End-to-end Kafka→blockchain latency",
...
class BlockchainLoggerWorker:
    ...
    def _process_message(self, raw_value: bytes) -> Optional[LogResult]:
        ...
        try:
            ...
            logger.error("Fabric submit failed for %s: %s", event.event_id, exc)
            raise
        ...
        logger.info("Logged %s tx=%s cid=%s %.1fms", event.event_id, tx_id, cid, duration_ms)
        return result
